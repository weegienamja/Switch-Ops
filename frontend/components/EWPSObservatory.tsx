...
const WORKLOADS = ["Background streaming", "Amazon Prime Video", "YouTube 4K", "Netflix", "Large download", "Normal browsing", "Idle baseline"];
  ...
  return (
    <section className="card ewps-lab" aria-labelledby="ewps-lab-title">
      ...
      <div className="ewps-lab__controls">
        ...
        {lab?.ready ? <><button className="btn" disabled={Boolean(busy)} onClick={() => void onAction("verify")}>Verify A + B</button><select aria-label="Controlled lab scenario" disabled={controlledSessionActive} value={scenario} onChange={(event) => setScenario(event.target.value as EWPSLabScenario)}>{LAB_SCENARIOS.map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select><button className="btn btn--primary" disabled={Boolean(busy) || controlledSessionActive} onClick={() => void onAction("prepare", scenario)}>Prepare scenario</button><button className="btn" disabled={Boolean(busy) || !controlledSessionActive || !lab.scenarioId || lab.scenarioPhase + 1 >= lab.scenarioPhaseCount} onClick={() => void onAction("advance")}>{lab.scenarioPhase + 1 >= lab.scenarioPhaseCount ? "Final phase active" : `Advance to Phase ${lab.scenarioPhase + 2}`}</button><button className="btn btn--ghost" disabled={Boolean(busy) || controlledSessionActive} onClick={() => void onAction("teardown")}>Teardown lab</button></> : null}
      </div>
      {lab?.scenarioId ? <div className="ewps-lab__phase" role="status"><strong>{LAB_SCENARIOS.find((item) => item.id === lab.scenarioId)?.label || lab.scenarioId}</strong><span>PHASE {lab.scenarioPhase + 1} OF {lab.scenarioPhaseCount}</span><b>{phaseLabel(lab.scenarioPhaseId)}</b><small>{controlledSessionActive ? "Authoritative for the running experiment" : "Prepared; no experiment or traffic steering was started"}</small></div> : null}
    </section>
  );
}
...
function ExperimentForm({ meta, candidates, lab, busy, onStart }: { meta: EWPSMeta; candidates: EWPSCandidatePath[]; lab: EWPSLabStatus | null; busy: boolean; onStart: (request: { name: string; workloadLabel: string; sourceMode: "REAL_INTERFACES" | "CONTROLLED_DUAL_PATH"; candidatePathIds: string[]; controlledScenario?: EWPSLabScenario | null; config: EWPSConfig }) => Promise<void> }) {
  ...
  return (
    <section className="card ewps-form">
      ...
      <div className="ewps-parameter-grid"><label><span>λ freshness decay</span><input aria-label="lambda freshness decay" type="number" step="0.005" min="0" value={config.lambda} onChange={(event) => parameter("lambda", Number(event.target.value))} /></label><label><span>k density rate</span><input aria-label="density rate" type="number" step="0.01" min="0.01" value={config.k} onChange={(event) => parameter("k", Number(event.target.value))} /></label><label><span>α performance risk</span><input aria-label="performance risk aversion" type="number" step="0.1" min="0" value={config.alpha} onChange={(event) => parameter("alpha", Number(event.target.value))} /></label><label><span>β topology bound</span><input aria-label="topology penalty beta" type="number" step="0.05" min="0" value={config.beta} onChange={(event) => parameter("beta", Number(event.target.value))} /></label><label><span>P perf min</span><input aria-label="minimum performance evidence threshold" type="number" step="0.05" min="0" max="1" value={config.pPerfMin} onChange={(event) => parameter("pPerfMin", Number(event.target.value))} /></label><label title={meta.sampleIntervalSemantics}><span>Start-to-start interval</span><input aria-label="sample interval" type="number" step="1" min="2" value={config.sampleIntervalSeconds} onChange={(event) => parameter("sampleIntervalSeconds", Number(event.target.value))} /></label></div>
...
function SummaryPanel({ summary, candidates }: { summary: EWPSSummary; candidates: EWPSCandidatePath[] }) {
  return (
    <section className="card ewps-summary">
      ...
      {summary.phaseSummaries?.length ? <div className="ewps-phase-summary"><h4>Scenario phase timeline</h4>{summary.phaseSummaries.map((phase) => <article key={`${phase.scenarioId}-${phase.phaseIndex}`}><div><strong>Phase {phase.phaseIndex} · {phaseLabel(phase.phaseId)}</strong><span>{duration(phase.durationSeconds)} · {phase.decisionPoints} decision points</span></div><small>{Object.entries(phase.measurementsPerPath).map(([pathId, count]) => `${pathLabel(pathId, candidates)} ${count} measurements, P perf ${number(phase.performanceConfidencePerPath[pathId]?.mean, 3)}, raw ${number(phase.rawCostDistributionPerPath[pathId]?.mean, 2)}, EWPS ${number(phase.ewpsCostDistributionPerPath[pathId]?.mean, 2)}, eligible ${duration(phase.pathEligibilitySeconds[pathId] || 0)}`).join(" · ")}</small><small>{phase.algorithmDisagreementCount} disagreement points · {phase.hysteresisSuppressions} hysteresis suppressions · {phase.telemetryFailures} failures · {phase.staleEvents} stale events</small></article>)}</div> : null}
...
export function EWPSObservatory() {
  ...
  const observedCadence = useMemo(() => {
    const records = timeline?.decisions.flatMap((point) => point.cadence ? [point.cadence] : []) || [];
    const intervals = records.flatMap((item) => item.actualStartToStartSeconds == null ? [] : [item.actualStartToStartSeconds]);
    return {
      meanSeconds: intervals.length ? intervals.reduce((sum, value) => sum + value, 0) / intervals.length : null,
      lastCollectionMs: records.at(-1)?.collectionDurationMs ?? null,
      overrunCount: records.at(-1)?.cadenceOverrunCount ?? 0,
    };
  }, [timeline]);
  const materialEvents = useMemo(() => {
    const decisionEvents = (timeline?.decisions || []).flatMap((point) => point.events.map((event) => ({
      key: `decision-${point.decisionIndex}-${event}`,
      timestamp: point.timestamp,
      title: event.replaceAll("_", " ").replaceAll(":", " · "),
      detail: "",
      failed: false,
    })));
    const phaseEvents = (timeline?.events || []).map((event) => ({
      key: event.eventId,
      timestamp: event.timestamp,
      title: event.applicationSucceeded ? "Scenario phase changed" : "Scenario phase apply failed",
      detail: `${phaseLabel(event.previousPhaseId)} → ${phaseLabel(event.newPhaseId)}`,
      failed: !event.applicationSucceeded,
    }));
    return [...decisionEvents, ...phaseEvents].sort((left, right) => right.timestamp.localeCompare(left.timestamp));
  }, [timeline]);
  ...
  async function labAction(action: "check" | "create" | "verify" | "prepare" | "advance" | "teardown", selectedScenario?: EWPSLabScenario) {
    setBusy(`lab-${action}`); setError(null);
    try {
      if (action === "advance") {
        const result = await api.ewpsLabAdvanceScenario();
        setLab(result.status);
        if (!result.event?.applicationSucceeded) setError(result.event?.detail || "Scenario phase application failed verification.");
        const [nextCandidates, nextTimeline] = await Promise.all([
          api.ewpsCandidates(),
          session ? api.ewpsTimeline(session.experimentId) : Promise.resolve(null),
        ]);
        setCandidates(nextCandidates);
        if (nextTimeline) setTimeline(nextTimeline);
      } else {
        const next = action === "check" ? await api.ewpsLabPrerequisites() : action === "create" ? await api.ewpsLabCreate() : action === "verify" ? await api.ewpsLabVerify() : action === "prepare" ? await api.ewpsLabPrepareScenario(selectedScenario || "faster-epistemically-weak") : await api.ewpsLabTeardown();
        setLab(next);
        setCandidates(await api.ewpsCandidates());
      }
    } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); }
    finally { setBusy(null); }
  }
  ...
  return (
    <div className="ewps-observatory">
      ...
      {session && !creatingNew ? <>
        ...
        <section className="card ewps-events"><div className="ewps-section-head"><div><span>MATERIAL EVENTS</span><h3>Impairment, evidence, availability, and decisions</h3></div><small>{materialEvents.length} markers</small></div><ol>{materialEvents.slice(0, 20).map((event) => <li className={event.failed ? "is-failed" : ""} key={event.key}><time>{new Date(event.timestamp).toLocaleTimeString()}</time><span>{event.title}{event.detail ? <small>{event.detail}</small> : null}</span></li>)}</ol>{!materialEvents.length ? <p className="ewps-empty">No impairment, staleness, failure, recovery, crossing, or suppression event is recorded yet.</p> : null}</section>
        {summary ? <SummaryPanel summary={summary} candidates={candidates} /> : null}
        {session.status === "COMPLETED" ? <section className="card ewps-research-actions"><div><span>REPRODUCIBILITY</span><h3>Replay and privacy-safe export</h3><p>The original session is immutable; stored model version selects v0.1 or v0.2 replay semantics.</p></div><div><button className="btn" disabled={busy === "replay"} onClick={() => void runReplay()}>Replay original</button>{(["jsonl", "json", "csv"] as const).map((format) => <button className="btn btn--ghost" disabled={busy === `export-${format}`} key={format} onClick={() => void exportExperiment(format)}>{busy === `export-${format}` ? "Saving…" : `Export ${format.toUpperCase()}`}</button>)}</div>{replay ? <><p className="ewps-replay-digest"><span>DETERMINISTIC DIGEST</span><code>{replay.deterministicDigest}</code> · {replay.decisions.length} reproduced decisions</p>{replay.events?.length ? <ol className="ewps-replay-phases"><li>{phaseLabel(replay.decisions[0]?.scenarioPhase?.phaseId || session.initialScenarioPhase?.phaseId)}</li>{replay.events.map((event) => <li key={event.eventId}><span>↓ phase transition · {new Date(event.timestamp).toLocaleTimeString()}</span><strong>{phaseLabel(event.newPhaseId)}</strong></li>)}</ol> : null}</> : null}</section> : null}
      </> : null}
