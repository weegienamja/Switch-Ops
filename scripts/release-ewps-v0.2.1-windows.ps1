# Build and stage the separate EWPS v0.2.1 Alpha Windows artifacts.

$ErrorActionPreference = "Stop"
$repoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$releaseRoot = Join-Path $repoRoot "release"
$stage = Join-Path $releaseRoot "ewps-v0.2.1-alpha"
...
foreach ($artifact in $installerFiles) {
    ...
    $name = "SwitchOps_EWPS_0.2.1-alpha_{0}{1}" -f $kind, $artifact.Extension
